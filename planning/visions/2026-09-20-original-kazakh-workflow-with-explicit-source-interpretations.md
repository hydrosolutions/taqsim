...
Order 179 paragraph 5 states that the main-river ecological flow equals the sum of the tributaries’ quantitative ecological-flow indicators. Preserve this attributed basin-accounting relation with supported tributary identities and quantities. It is not a rule to sum repeated requirements at successive sections or a substitute for routing releases. Preserve explicit prepared topology and avoid double-counting the same water. If topology or accounting evidence cannot support the source relation, retain that limitation explicitly; do not replace a missing physical mapping with a universal routing sum or maximum.
...
- Receiving-parent topology differs from nearest gauge and numerical-order heuristics. Exercise accepted donor, missing donor/coefficient and further-study routes; verify mouth-to-source processing, an explicit supported paragraph-5 tributary-summation case, missing-accounting evidence, and no duplicate consumptive accounting across successive reaches.
