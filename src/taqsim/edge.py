...
from .validation import validate_nonnegativity_int_or_float, validate_probability
...
class Edge:
    ...
    def __init__(
        ...
        if ecological_flow > capacity:
            raise ValueError(
                f"Ecological flow from {source} to {target} ({ecological_flow}) cannot exceed capacity {capacity}"
            )
        ...
        if length is not None:
            ...
            try:
                ...
                logger.warning("Could not calculate edge length from coordinates. Using length = 0.")
                self.length = 0
    ...
    def update(self, flow: float | None = None) -> None:
        """
        ...
        try:
            ...
            logger.error(f"Error updating edge flow: {str(e)}. Flow values may be incorrect.")
    ...
    def get_edge_length(self) -> float:
        ...
        try:
            ...
            if any(
                attr is None
                for attr in [self.source.easting, self.source.northing, self.target.easting, self.target.northing]
            ):
            ...
            if length == 0:
                logger.warning("Calculated edge length is 0 - source and target at same location")
