...
def get_node_outflow(node, time_step):
    ...
    from .nodes import DemandNode, HydroWorks, RunoffNode, StorageNode, SupplyNode
...
class WaterSystemVisualizer:
    ...
    def __init__(self, water_system, name="", clock: Callable[[], datetime] | None = None):
        """
        ...
        Args:
            ...
            clock (Callable[[], datetime] | None): Optional clock function for testable timestamps
        """
        ...
        self.timestamp = self._clock().strftime("%Y%m%d_%H%M%S")
        ...
        for time_step in range(self.system.time_steps):
            ...
            for node_id, node_data in self.system.graph.nodes(data=True):
                ...
                try:
                    ...
                    inflow = (
                        sum(edge.flow_after_losses[time_step] for edge in node.inflow_edges.values())
                        if hasattr(node, "inflow_edges")
                        else 0
                    )
                    ...
                    if isinstance(node, SupplyNode):
                        ...
                        storage_change = (
                            node.storage[time_step + 1] - node.storage[time_step]
                            if time_step + 1 < len(node.storage)
                            else 0
                        )
                        row_data[f"{node_id}_StorageChange"] = storage_change
                        row_data[f"{node_id}_ExcessVolume"] = (
                            node.spillway_register[time_step] if time_step < len(node.spillway_register) else 0
                        )
                    elif isinstance(node, DemandNode):
                        row_data[f"{node_id}_Demand"] = node.demand_rates[time_step]
                        row_data[f"{node_id}_SatisfiedDemand"] = (
                            node.satisfied_consumptive_demand[time_step]
                            if time_step < len(node.satisfied_consumptive_demand)
                            else 0
                        )
                    ...
                    logger.warning(f"Error processing node {node_id} at time step {time_step}: {str(e)}")
            ...
            for u, v, d in self.system.graph.edges(data=True):
                ...
                try:
                    ...
                    logger.warning(f"Error processing edge {u}->{v} at time step {time_step}: {str(e)}")
    ...
    def plot_reservoir_dynamics(self):
        ...
        if not storage_nodes:
            logger.info("No storage nodes found in the system.")
            return None
        ...
        for idx, (node_id, node) in enumerate(storage_nodes):
            ...
            inflows = [sum(edge.flow_after_losses[t] for edge in node.inflow_edges.values()) for t in time_steps]
            ...
            ax1.plot(time_steps, inflows, color=colors["inflow"], label="Inflow", linewidth=2)
            ax1.plot(time_steps, outflows, color=colors["outflow"], label="Outflow", linewidth=2)

            # Plot waterlevel on right y-axis
            ax2.plot(
                time_steps, waterlevels, color=colors["waterlevel"], label="Waterlevel", linewidth=2, linestyle="--"
            )
            ...
            ax1.legend(lines1 + lines2, labels1 + labels2, loc="center right", bbox_to_anchor=(1.3, 0.5))
    ...
    def plot_spills(self):
        ...
        if not hydroworks_nodes and not reservoir_nodes:
            logger.info("No hydroworks or reservoir nodes found in the system.")
            return None
        ...
        for idx, (node_id, node) in enumerate(hydroworks_nodes):
            if hasattr(node, "spill_register") and len(node.spill_register) > 0:
                ...
                line = ax1.plot(
                    time_steps, spill_rates, color=colors[idx], label=node_id, linewidth=2, marker="o", markersize=4
                )
    ...
    def plot_network_overview(self):
        ...
        grouped_nodes = {
            node_type: [
                node for node, data in self.system.graph.nodes(data=True) if isinstance(data["node"], node_type)
            ]
        ...
        for u, v, edge_data in self.system.graph.edges(data=True):
            ...
            ax.plot(
                ...
                path_effects=[PathEffects.withStroke(linewidth=width + 1, foreground="white", alpha=0.2)],
            )
        ...
        edge_labels = {
            (u, v): f"{edge_data['edge'].capacity:.0f}"
            for u, v, edge_data in self.system.graph.edges(data=True)
        ...
        for node, label in node_labels.items():
            x, y = pos[node]
            txt = ax.text(x, y, label, fontsize=15, ha="center", va="center", fontweight="bold", zorder=5)
        ...
        legend_elements.append(Line2D([0], [0], color=edge_cmap(0.5), linewidth=3, label="Edge"))

        # Add the legend
        ax.legend(handles=legend_elements, loc="upper right", fontsize=22, framealpha=0.9, fancybox=True, shadow=True)
    ...
    def plot_minimum_flow_compliance(self):
        ...
        sink_nodes_with_requirements = [
            (id, node) for id, node in sink_nodes if any(flow > 0 for flow in node.min_flows)
        ]

        if not sink_nodes_with_requirements:
            logger.info("No sink nodes with minimum flow requirements found in the system.")
            return None
        ...
        for idx, (node_id, node) in enumerate(sink_nodes_with_requirements):
            ...
            # Plot flows
            ax.plot(time_steps, actual_flows, color=colors["actual"], label="Modelled Flow", linewidth=2)
            ax.plot(time_steps, min_flows, color=colors["minimum"], label="Min Requirement", linewidth=2)
            ...
            compliance_rate = sum(1 for a, m in zip(actual_flows, min_flows) if a >= m) / len(time_steps) * 100

            stats_text = f"Total Deficit Volume: {total_deficit_volume:,.0f} m³"
    ...
    def plot_demand_deficit_heatmap(self):
        ...
        if not demand_nodes:
            logger.info("No demand nodes found in the system.")
            return
    ...
    def plot_reservoir_volumes(self):
        ...
        if not storage_nodes:
            logger.info("No storage nodes (reservoirs) found in the system.")
            return None
        ...
        for idx, (node_id, node) in enumerate(storage_nodes):
            ...
            if len(volumes) < self.system.time_steps + 1:
                ...
                volumes = np.append(
                    volumes, [volumes[-1] if len(volumes) > 0 else 0] * (self.system.time_steps - len(volumes))
                )
            ...
            line1 = ax1.plot(time_steps, volumes, color="blue", label="Storage Volume", linewidth=2)

            # Add capacity line
            cap_line = ax1.axhline(y=capacity, color="red", linestyle="--", label="Maximum Storage Capacity")
    ...
    def plot_network_layout(self):
        ...
        for node_id, data in self.system.graph.nodes(data=True):
            node = data["node"]
            if (
                node_id in SinkNode.all_ids or node_id in DemandNode.all_ids  # or
    ...
    def plot_system_demands_vs_inflow(self):
        ...
        try:
            ...
            ax1.plot(
                time_steps, total_min_flows, "darkorange", label="Min Flow Requirement", linewidth=2, linestyle="--"
            )
            ...
            total_only_deficit = sum(
                max(0, (b - a)) * self.system.dt for a, b in zip(total_inflows, total_requirements)
            )
            total_only_surplus = sum(
                max(0, (a - b)) * self.system.dt for a, b in zip(total_inflows, total_requirements)
            )
            ...
            logger.error(f"Error plotting system demands vs inflow: {str(e)}")
            return None
    ...
    def plot_interactive_network(self, filename="interactive_network.html"):
        ...
        """
        # Get node positions
        pos = {node: (data["node"].easting, data["node"].northing) for node, data in self.system.graph.nodes(data=True)}
        ...
        for t in time_steps:
            ...
            edge_trace = go.Scatter(
                x=edge_x, y=edge_y, line=dict(width=2, color="royalblue"), hoverinfo="none", mode="lines"
            )
        ...
        logger.info(f"Interactive network visualization saved to {os.path.join(self.image_dir, filename)}")
        return fig
