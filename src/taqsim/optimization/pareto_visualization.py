...
class ParetoVisualizer:
    ...
    def _pareto_front_3d(self, filename):
        ...
        fig = make_subplots(
            ...
            subplot_titles=["3D Pareto Front", f"{y} vs {x}", f"{y} vs {z}", f"{x} vs {z}"],
        )
    ...
    def _pareto_front_nd(self, filename):
        ...
        fig = make_subplots(
            rows=n_rows, cols=n_cols, subplot_titles=subplot_titles, horizontal_spacing=0.15, vertical_spacing=0.15
        )
        ...
        for idx, (x, y) in enumerate(pairs):
            ...
            fig.add_trace(
                go.Scatter(
                    x=[self.df.loc[min_x_idx, x]],
                    y=[self.df.loc[min_x_idx, y]],
                    mode="markers",
            ...
            fig.add_trace(
                go.Scatter(
                    x=[self.df.loc[min_y_idx, x]],
                    y=[self.df.loc[min_y_idx, y]],
                    mode="markers",
        ...
        fig.update_layout(
            title="All 2D Pareto Fronts for Objective Pairs", height=400 * n_rows, width=500 * n_cols, showlegend=False
        )
        output_path = os.path.join(self.output_dir, filename)
        fig.write_html(output_path)
    ...
    def create_representative_table(self, filename="representative_solutions.html"):
        ...
        fig = go.Figure(
            data=[
                go.Table(
                    header=dict(values=display_columns, fill_color="paleturquoise", align="left", font=dict(size=12)),
                    cells=dict(
        ...
        fig.update_layout(title="Representative Solutions", height=350, width=1200)
        output_path = os.path.join(self.output_dir, filename)
        fig.write_html(output_path)
    ...
    def create_parallel_coordinates_plot(self, filename="parallel_coordinates.html"):
        ...
        fig.update_layout(font=dict(size=12), height=600, width=800)
