...
def initialize_time_series(
    id: str, csv_file: str, start_year: int, start_month: int, num_time_steps: int, column_name: str = "Q"
) -> list[float]:
    """
...
class SupplyNode:
    ...
    def __init__(
        ...
        easting: float,
        northing: float,
        constant_supply_rate: float | None = None,
        ...
        if all(param is not None for param in [csv_file, start_year, start_month, num_time_steps]):
            imported_data = initialize_time_series(id, csv_file, start_year, start_month, num_time_steps, "Q")
...
class SinkNode:
    ...
    def __init__(
        ...
        easting: float,
        northing: float,
        constant_min_flow: float | None = None,
        csv_file: str | None = None,
        start_year: int | None = None,
        start_month: int | None = None,
        num_time_steps: int = 0,
...
class DemandNode:
    ...
    def __init__(
        ...
        constant_demand_rate: float | None = None,
        ...
        """
        # Validate Node ID and coordinates
        validate_node_id(id, "DemandNode")
        validate_coordinates(easting, northing, id)

        # Validate and store parameters
        priority, field_efficiency, conveyance_efficiency, non_consumptive_rate = self._validate_demand_params(
            priority, field_efficiency, conveyance_efficiency, non_consumptive_rate
        )

        # Initialize basic attributes
        self.id = id
        DemandNode.all_ids.append(id)
        self.easting = easting
        self.northing = northing
        self.inflow_edges = {}
        self.outflow_edge = None
        ...
        # Initialize demand rates
        demand_rates = self._initialize_demand_rates(
            csv_file, constant_demand_rate, start_year, start_month, num_time_steps
        )

        # Apply efficiency factors
        self.demand_rates = self._apply_efficiency_factors(demand_rates, field_efficiency, conveyance_efficiency)

    def _validate_demand_params(
        self, priority: int, field_eff: float, conv_eff: float, non_consumptive: float
    ) -> tuple[int, float, float, float]:
        """
        Validate demand node parameters.

        Args:
            priority: Priority indicator (must be 1 or 2)
            field_eff: Field efficiency (must be between 0 and 1)
            conv_eff: Conveyance efficiency (must be between 0 and 1)
            non_consumptive: Non-consumptive rate (must be non-negative)

        Returns:
            Validated parameters as tuple
        ...
        validate_probability(field_eff, "field_efficiency")
        validate_probability(conv_eff, "conveyance_efficiency")
        validate_nonnegativity_int_or_float(non_consumptive, "non_consumptive_rate")

        return priority, field_eff, conv_eff, non_consumptive

    def _register_priority(self, priority: int) -> None:
        """
        Register node ID in appropriate priority tracking list.

        Args:
            priority: Priority indicator (1=high, 2=low)
        """
        if priority == 1:
            DemandNode.high_priority_demand_ids.append(self.id)
        else:
            DemandNode.low_priority_demand_ids.append(self.id)

    def _initialize_demand_rates(
        self,
        csv_file: str | None,
        ...
        """
        Initialize demand rates from CSV, constant value, or zeros.

        Args:
            csv_file: Path to CSV file with time-varying demand data
            constant_rate: Constant demand rate [m³/s]
            start_year: Starting year for CSV data
            start_month: Starting month for CSV data
            num_time_steps: Number of time steps to simulate

        Returns:
            List of demand rates (before efficiency adjustment)

        Raises:
            ValueError: If imported data is invalid
        """
        # Try to import time series data first
        imported_data = None
        if all(param is not None for param in [csv_file, start_year, start_month, num_time_steps]):
            imported_data = initialize_time_series(self.id, csv_file, start_year, start_month, num_time_steps, self.id)
        ...
        elif constant_rate is not None:
            # Validate constant_demand_rate
            validate_nonnegativity_int_or_float(constant_rate, "constant_demand_rate")
            if constant_rate < self.non_consumptive_rate:
                raise ValueError("Demand rate cannot be less than non-consumptive rate")
            return [constant_rate] * num_time_steps

        else:
            # Default to zero demand if no other information provided
            return [0] * num_time_steps

    def _apply_efficiency_factors(self, rates: list[float], field_eff: float, conv_eff: float) -> list[float]:
        """
        Apply efficiency factors to demand rates.

        Args:
            rates: Base demand rates [m³/s]
            field_eff: Field efficiency (0-1)
            conv_eff: Conveyance efficiency (0-1)

        Returns:
            Adjusted demand rates accounting for inefficiencies

        Note:
            When efficiencies < 1, gross water requirements increase
            to satisfy the same net demand.
        """
        efficiency_product = field_eff * conv_eff
        return [rate / efficiency_product for rate in rates]
    ...
    def update(self, time_step: int, dt: float) -> None:
        ...
        try:
            ...
            total_inflow = sum(edge.flow_after_losses[time_step] for edge in self.inflow_edges.values())
            current_demand = self.demand_rates[
                time_step
            ]  # Total demand for this timestep (consumptive + non-consumptive)
...
class StorageNode:
    ...
    def __init__(
        ...
        northing: float,
        hv_file: str,
        evaporation_file: str | None = None,
        start_year: int | None = None,
        start_month: int | None = None,
        num_time_steps: int = 0,
        ...
        imported_data = None
        if evaporation_file is not None and all(
            param is not None for param in [start_year, start_month, num_time_steps]
        ):
    ...
    def set_release_params(self, params: dict[str, float | list[float]]) -> None:
        """
        ...
        for month in range(12):
            ...
            if self.dead_storage >= V1:
                raise ValueError(
                    f"Month {month + 1}: V1 ({V1}) must be greater than dead storage ({self.dead_storage})"
                )
    ...
    def update(self, time_step: int, dt: float) -> None:
        ...
        try:
            ...
            new_water_level = max(
                (previous_water_level - (self.evaporation_rates[time_step] / 1000)), self.hv_data["min_waterlevel"]
            )  # Convert mm to m
            ...
            logger.error(f"Error updating storage node {self.id}: {str(e)}")
...
class HydroWorks:
    ...
    def set_distribution_parameters(self, parameters: dict[str, float | list[float]]) -> None:
        """
        ...
        for node_id, params in parameters.items():
            if isinstance(params, (int, float)):
                ...
                raise ValueError(
                    f"Parameters for edge to {node_id} must be either a single value or a list of 12 monthly values"
                )

        # Verify all parameters are valid
        for node_id, params in new_params.items():
            if not np.all((params >= 0) & (params <= 1)):
                raise ValueError(f"Distribution parameters for edge to {node_id} must be between 0 and 1")

        # Verify parameters sum to 1 for each month
        for month in range(12):
            total = sum(params[month] for params in new_params.values())
            total += sum(
                self.distribution_params[node_id][month] for node_id in self.outflow_edges if node_id not in new_params
            )
...
class RunoffNode:
    ...
    def __init__(
        ...
        self.rainfall_data = (
            initialize_time_series(id, rainfall_csv, start_year, start_month, num_time_steps, "Precipitation")
            if rainfall_csv
            else []
        )
        ...
        if not self.rainfall_data and num_time_steps:
            ...
            logger.warning(f"No rainfall data provided for RunoffNode '{id}'. Initializing with zeros.")
    ...
    def update(self, time_step: int, dt: float) -> None:
        ...
        try:
            ...
            logger.error(f"Error updating RunoffNode {self.id}: {str(e)}")
